...
public class HungerGames {
    ...
    public void setupDistricts (String filename) {
        int numberOfDistrects = StdIn.readInt();

        for (int i = 0; i < numberOfDistrects; i++) {
    ...
    public void setupPeople (String filename) {
        int numberOfTributes = StdIn.readInt();

        for (int i = 0; i < numberOfTributes; i++) {
            String[] data = StdIn.readString().split(" ");
            ...
            Person tribute = new Person(birthMonth, firstName, lastName, age, districtId, effectiveness);

            int districtIndex = 0;

            while (districtIndex != districtId) {
                districtId++;
            }

            if (birthMonth % 2 == 0) {
                districts.get(districtIndex).addEvenPerson(tribute);
            } else {
                districts.get(districtIndex).addOddPerson(tribute);
            }
        }
    }
